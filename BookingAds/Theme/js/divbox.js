(function ($) {
  $.fn.divbox = function (opt) {
    var _cfg = {
      ...
      api: { start: null, beginLoad: null, afterLoad: null, closed: null },
    };
    ...
    function _run(index, init) {
      ...
      fn.closed = function () {
        $("#divbox_frame").animate(
          { top: _click.top, left: _click.left, width: "0px", height: "0px" },
          _cfg.speed,
          function () {
            $(this).remove();
            $("#divbox").remove();
      ...
      fn.init = function () {
        ...
        $("#divbox_frame")
          ...
          .animate({
            ...
            height: 50,
            top: _cfg.top
              ? sizesystem[3] + _cfg.top
              : sizesystem[3] + Math.round(sizesystem[5] / 2),
            left: _cfg.left
              ? sizesystem[2] + _cfg.left
              : Math.round(sizesystem[4] / 2),
          });
      };
      fn.resizeWindow = function (resizeW) {
        ...
        $(oFrame).css({
          top: _cfg.top
            ? sizesystem[3] + _cfg.top
            : sizesystem[3] + Math.round((sizesystem[5] - h) / 2),
          left: _cfg.left
            ? sizesystem[2] + _cfg.left
            : Math.round((sizesystem[4] - w) / 2),
        });
      };
        ...
        $(oFrame)
          ...
          .animate(
            ...
            function () {
              ...
              if (
                _cfg.caption != false &&
                (caption != "" || _cfg.caption_control == true)
              ) {
                ...
                $(oFrame)
                  ...
                  .find(".caption")
                  .show();
                if (_cfg.caption_control == true) {
                  var btn_top = h + parseInt(c.css("padding-top"));
                  $(_cfg.btn_prev + "," + _cfg.btn_next + "," + _cfg.btn_number)
                    .css({ top: btn_top })
                    .show();
                  if (index * 1 > 0) {
                    $(_cfg.btn_prev)
              ...
              try {
                ...
                if (!_cfg.full_drag_handle)
                  $("#divbox_content").css({ cursor: "pointer" });
              } catch (e) {}
          ...
          fn.animate(
            ...
            function (o) {
              if (ie6 && ext == "png") {
                ...
                $(o)
                  .find("img")
                  .css({ width: imgW, height: imgH, display: "block" })
                  .fadeIn();
              }
              if (zoom)
                $(o)
                  .find("img")
                  .addClass("zoom")
                  .attr({ title: _cfg.languages.click_full_image })
                  .click(function () {
      ...
      fn.flashEmbedString = function (file, w, h, type) {
      ...
      fn.viewYouTube = function (src, caption) {
        ...
        var vidSrc =
          "http://www.youtube.com/v/" + vidId + "&hl=en&fs=1&autoplay=1&rel=0";
        var winW = 640;
        var winH = 385;
        var top = sizesystem[3] + Math.round((sizesystem[5] - winH) / 2);
        var left = Math.round((sizesystem[4] - winW) / 2);
